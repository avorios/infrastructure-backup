...
### Required evidence for Trust & Safety claims

Before posting any comment claiming a policy update, safety rule change, or content moderation fix:

1. **File existence:** `ls -la <path>` for every file you changed — paste the output.
2. **Content excerpt:** Paste the actual content you added or changed — not "I updated the policy" but the policy text itself.

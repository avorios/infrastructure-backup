...
3. **Query USAC for the allowable contract date:**
   - Call `get_470_filings(application_number)` to get filing metadata
   - Extract `allowable_contract_date` or calculate from `posting_date + 28 days`
   - Include this date in the bid sheet header
4. Apply HITCO's pricing model based on category, service type, and scope:
...
5. Build a Bid Sheet with line items, totals, and discount rate calculations
6. **Bid sheet must include:** application number, allowable contract date, and category
7. Save bid sheet to `/Users/ivoryrobinson/erate-workspace/bids/`
8. Post bid summary as an issue comment
